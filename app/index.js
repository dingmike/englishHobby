...
// getAccessToken(); // 获取微信accessoken 开启后每隔2小时更新一次access_token
...
let connection = mysql.createConnection(config.mysqlDBconfig);
...
//app.use(compression());
    ...
    server.on('listening', function () {
        logger.writeInfo('Express server listening on port ' + server.address().port + " with pid " + process.pid + ' Servers running at ' + 'http://' + app.get('ipaddr') + ':' + app.get('port'));
    ...
    servers.on('listening', function () {
        logger.writeInfo('Express server listening on port ' + server.address().port + " with pid " + process.pid + ' ServerSSL running at ' + 'https://' + app.get('ipaddr') + ':' + app.get('portSSL'));
        // log.info('Express server listening on port ', server.address().port, " with pid ", process.pid );
